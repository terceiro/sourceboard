module SourceBoard
  ...
  class Parser
    ...
    # element of the source code, and must have at least the following (symbol)
    # keys:
    #
    ...
    #
    # +:metrics+::  a hash containing metrics for the given element. +:loc+ and
    #               +:number_of_definitions+ are mandatory metrics and
    #               therefore these keys must be present, but a parser may
    #               provide any other metrics. The hash values must be numbers.
    #
    ...
    #           editor. If omitted, the editor will open with the cursor at its
    #           default position (usually the first line of the file).
    #
    ...
    # +number_of_definitions+ (a generalization of "number of methods") must be
    # supported.
    #
